  ...
  function renderHeader(meta, minutes) {
    ...
    return `
      ...
      <div class="post-meta">
        ...
        ${
          tags.length
            ? `<span>•</span><span class="flex flex-wrap gap-2">${chips}</span>`
            : ""
        }
      </div>
    ...
    try {
      ...
      if (!res.ok) {
        document.getElementById(
          "post"
        ).innerHTML = `<p class="text-red-600">Post not found (HTTP ${res.status}).<br>
        ...
        const obs = new IntersectionObserver(
          (entries) => {
            entries.forEach((e) => {
              ...
              if (link)
                link.style.fontWeight = e.isIntersecting ? "700" : "400";
            });
          },
      ...
      function updateProgress() {
        ...
        const scrolled = Math.min(
          Math.max(window.scrollY - postEl.offsetTop, 0),
          total
        );
